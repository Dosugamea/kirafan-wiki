...
async function parseCommand(acb, command, key) {
  ...
  while (k < command.length) {
    const cmd = command.readUInt16BE(k);
    k += 2;
    const len = command.readUInt8(k);
    k += 1;
    ...
    switch (cmd) {
      ...
      case 0x07d0: // Start Waveform
        u16 = command.readUInt16BE(k);
        k += 2;
        if (u16 !== 0x0002) debugger;
        const SynthIndex = command.readUInt16BE(k);
        k += 2;
        ...
        if (Waveform.EncodeType === 2) {
          ...
          if (Buffer.isBuffer(file)) {
            ...
            if (isMemory) acb.memoryHcas[Waveform.MemoryAwbId] = file;
            else
              acb.streamHcas[Waveform.StreamAwbPortNo][Waveform.StreamAwbId] =
                file;
            if (samplingRate === 0) samplingRate = file.samplingRate;
            else if (samplingRate !== file.samplingRate)
              throw new Error(`SamplingRate Different`);
            if (channelCount === 0) channelCount = file.channelCount;
            else if (channelCount !== file.channelCount)
              throw new Error(`ChannelCount Different`);
          }
        } else {
      ...
      case 0x07d1: // Set Position
        const StartOffset = command.readUInt32BE(k);
        k += 4;
    ...
    if (Cue.ReferenceType === 1) {
      ...
      for (let j = 0; j < Sequence.NumTracks; j++) {
        ...
        if (track.samplingRate) {
          if (samplingRate === 0) samplingRate = track.samplingRate;
          else if (track.samplingRate !== samplingRate)
            throw new Error(`SamplingRate Different`);
        }
        if (track.channelCount) {
          if (channelCount === 0) channelCount = track.channelCount;
          else if (track.channelCount !== channelCount)
            throw new Error(`ChannelCount Different`);
        }
        let time = 0;
          ...
          switch (command.type) {
            case 0:
              ...
              let offset = Math.round(
                (time * samplingRate * channelCount) / 1000
              );
              if (offset % channelCount !== 0)
                offset += channelCount - (offset % channelCount);
              if (m == timeline.length)
                timeline.push({ time, offset, pcmDatas: [] });
      ...
      for (let i = 0; i < timeline.length; i++) {
        ...
        if (pcmDatas.length > 0) {
          let max = 0;
          for (let j = 1; j < pcmDatas.length; j++)
            if (pcmDatas[j].length > max) max = j;
